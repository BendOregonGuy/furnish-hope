...
export const TABLE_OVERRIDES: Record<string, TableOverride> = {
  ...
  tbl_client_request_items: {
    ...
    // item_name is the primary label now; the item_category link is optional
    // (kept for valuation), so fall back to it only when there's no name.
    displaySql: "COALESCE(NULLIF(t.item_name,''), (SELECT ic.item_category FROM lkp_item_category ic WHERE ic.item_category_id = t.item_category_id), 'Item') || ' ×' || t.quantity::text",
    listColumns: ['client_request_items_id', 'client_provisioning_request_id', 'item_name', 'room', 'quantity', 'qty_given'],
    searchColumns: ['item_name', 'item_notes', 'room'],
    columns: {
      item_notes: { type: 'textarea', label: 'Notes' },
      item_name: { label: 'Item name' },
      room: { label: 'Room' },
      qty_given: { label: 'Qty given' },
      pulled: { label: 'Pulled' },
      is_na: { label: 'N/A' },
      is_declined: { label: 'Declined' },
      time_stamp: { label: 'Created at' },
    },
  },
  tbl_request_child: {
    group: 'Clients & Referrals',
    label: 'Packing List — Children',
    singular: 'Packing List Child',
    displaySql: "COALESCE(t.gender,'child') || COALESCE(' • age ' || t.age::text, '')",
    listColumns: ['request_child_id', 'client_provisioning_request_id', 'gender', 'age'],
    searchColumns: ['notes'],
    columns: {
  ...
  tbl_packing_template_room: {
    group: 'Clients & Referrals',
    label: 'Packing Template — Rooms',
    singular: 'Template Room',
    displaySql: "t.room_name",
    listColumns: ['packing_template_room_id', 'room_name', 'sort_order', 'is_active'],
    searchColumns: ['room_name'],
    defaultSort: { column: 'sort_order', direction: 'asc' },
  },
  tbl_packing_template_item: {
    group: 'Clients & Referrals',
    label: 'Packing Template — Items',
    singular: 'Template Item',
    displaySql: "t.item_name",
    listColumns: ['packing_template_item_id', 'packing_template_room_id', 'item_name', 'default_qty', 'sort_order', 'is_active'],
